...
export const TokenDetails = () => {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      {(row.anprImageRef || row.loadImageRef) && (
        <div className="border rounded-md bg-card p-4">
          <div className="text-xs font-medium uppercase text-muted-foreground tracking-wider mb-3">
            Captured Images
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <ImageTile label="Front (ANPR)" src={resolveImageSrc(row.anprImageRef)} caption={row.anprNumberPlateText || undefined} />
            <ImageTile label="Top (Load)" src={resolveImageSrc(row.loadImageRef)} />
          </div>
        </div>
      )}
...
function ImageTile({ label, src, caption }: { label: string; src: string | null; caption?: string }) {
  return (
