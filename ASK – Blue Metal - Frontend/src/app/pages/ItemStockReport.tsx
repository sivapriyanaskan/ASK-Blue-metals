  ...
  const load = async () => {
    ...
    try {
      const [itemsRes, salesRes, purchaseRes, rmRes] = await Promise.all([
        ...
        salesBillApi.list({ pageSize: 200, status: 'POSTED' }),
        purchaseBillApi.list({ pageSize: 200, status: 'POSTED' } as any),
        rawMaterialEntryApi.list({ pageSize: 200 }),
      ]);
  ...
  return (
    <div className="p-6">
      <div className="mb-4 pb-3 border-b border-gray-300 flex items-center justify-between">
        ...
        <button onClick={load} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-1.5 rounded text-sm">Refresh</button>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-300">
                <tr>{['Code','Item Name','Type','Purchased (T)','Sold (T)','Consumed (T)','Closing Stock (T)','Selling Price'].map(h=><th key={h} className="px-3 py-2 text-left text-xs font-semibold text-gray-600 whitespace-nowrap">{h}</th>)}</tr>
              </thead>
              <tbody>
