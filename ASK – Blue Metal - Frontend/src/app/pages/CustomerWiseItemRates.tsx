...
interface RateRow {
  ...
  existingId: string | null; // active rate row id, if the customer already has one
}
...
export const CustomerWiseItemRates = () => {
  ...
  const openCreate = () => {
    setBulkCustomerId(''); setBulkValidFrom(''); setBulkValidTo(''); setRows([]); setRowSearch('');
    setError(null); setMode('create');
  };

  // Load the customer's existing rates and pre-fill an input row for every item.
  const onSelectCustomer = async (customerId: string) => {
    ...
    try {
      const res = await customerRatesApi.list({ customerId, pageSize: 500 });
      const activeByItem = new Map<string, CustomerRateRow>();
  ...
  const handleBulkSave = async () => {
    if (!bulkCustomerId) { setError('Please select a customer first.'); return; }
    // Only rows where the value was entered/changed (cleared rows are ignored).
    ...
    try {
      const results = await Promise.allSettled(changed.map((r) => {
        ...
        if (r.existingId) {
          return customerRatesApi.update(r.existingId, {
            rate,
            ...(validFrom !== undefined ? { validFrom } : {}),
            ...(validTo !== undefined ? { validTo } : {}),
          });
        }
        return customerRatesApi.create({ customerId: bulkCustomerId, itemId: r.itemId, rate, validFrom, validTo });
      }));
    ...
    return (
      <div className="p-6">
        ...
        <div className="mb-6 text-center">
          <h1 className="text-2xl font-bold text-gray-900">Edit Customer Rate</h1>
        </div>
  ...
  // Bulk create — set rates for many items of one customer in a single screen.
  return (
    <div className="p-6">
      <button onClick={() => setMode('list')} className="inline-flex items-center gap-2 text-blue-600 hover:text-blue-700 mb-4">
        <ArrowLeft className="w-4 h-4" /> Back to List
      ...
      <div className="mb-6 text-center">
        <h1 className="text-2xl font-bold text-gray-900">Add Customer Rates</h1>
        <p className="text-sm text-gray-500 mt-1">Pick a customer, then set rates for any number of items at once.</p>
      </div>
      ...
      <div className="max-w-4xl mx-auto bg-white rounded-lg border border-gray-300 p-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          <div className="md:col-span-1">
            ...
            <SearchableDropdown options={customerOptions} value={bulkCustomerId} onValueChange={onSelectCustomer} placeholder="Select customer" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Valid From</label>
        ...
        {!bulkCustomerId && <div className="text-center py-12 text-gray-500">Select a customer to load items.</div>}
        {bulkCustomerId && loadingRows && <div className="text-center py-12 text-gray-500 flex items-center justify-center gap-2"><Loader2 className="w-4 h-4 animate-spin" /> Loading items…</div>}

        {bulkCustomerId && !loadingRows && (
          <>
        ...
        <div className="flex gap-4 mt-6">
          <button onClick={handleBulkSave} disabled={saving || !bulkCustomerId} className="flex-1 px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:bg-gray-400 flex items-center justify-center gap-2 font-medium">
            {saving ? <Loader2 className="w-5 h-5 animate-spin" /> : <Save className="w-5 h-5" />}
            Save Rates
          </button>
          <button onClick={() => setMode('list')} className="px-6 py-3 border border-gray-300 rounded-lg hover:bg-gray-50 font-medium">Cancel</button>
