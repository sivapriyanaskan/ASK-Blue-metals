...
export const SupplierWiseItemRates = () => {
  ...
  useEffect(() => {
    Promise.all([suppliersApi.list({ pageSize: 200 }), itemsApi.list({ pageSize: 200 })])
  ...
  const handleSave = async () => {
    if (!form.supplierId || !form.itemId || !form.rate) { setError('Supplier, item and rate are required.'); return; }
    ...
    try {
      if (editingId) {
        ...
        await supplierRatesApi.create({
          supplierId: form.supplierId, itemId: form.itemId, rate: rateNum,
      ...
      <div className="p-6">
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Purchase rates per supplier per item with validity windows.</p>
          </div>
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-6 text-center">
        <h1 className="text-2xl font-bold text-gray-900">{mode === 'create' ? 'Add Supplier Rate' : 'Edit Supplier Rate'}</h1>
      </div>
      ...
      <div className="max-w-2xl mx-auto bg-white rounded-lg border border-gray-300 p-6">
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="col-span-2">
            ...
            <SearchableDropdown options={supplierOptions} value={form.supplierId} onValueChange={(v) => setForm({ ...form, supplierId: v })} placeholder="Select supplier" disabled={mode === 'edit'} />
          </div>
          <div className="col-span-2">
